...
| 9 | 도감 연동 리뷰·다축 별점(하이브리드) + 커뮤니티 후기 아이템 태깅 | `363becc` |
...
- **Playwright E2E `e2e/09-mark-all-read.spec.ts`: 1/1 ✅(라이브)** — 안 읽음 3건 시드 → 종 열고 **"모두 읽음" → 배지 즉시 0 + DB is_read=true(서버 저장) + 리로드 후에도 0 유지**. Phase 7 함정(폼 언마운트 액션 취소→startTransition 회피)이 프로덕션에서 동작 확인.
...
- [x] **벌점제(Phase 8) — 운영자 부과/해제 UI**: e2e `06`이 2계정으로 검증(부과→누적·중복차단·종 배지·/me 배너 + admin **제재 관리** 섹션에서 영구정지 **해제→차단 풀림**). 데이터=verify-phase8, 정지 게이트=e2e `05`. 실시간 종 푸시=e2e `07`(라이브). → **Phase 8 전 영역 자동 검증 완료.**
...
- [x] **알림 Realtime(새로고침 없이 종 배지 증가)**: 라이브에서 e2e `07`(직접 INSERT)·`08`(**2계정 댓글/좋아요**)로 검증.
- [x] **"모두 읽음" → 배지 0 + 재방문 유지**: 라이브에서 e2e `09`로 검증(즉시 0·DB 저장·리로드 유지).
